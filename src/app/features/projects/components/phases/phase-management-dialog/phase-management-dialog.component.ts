            ...
            <div class="phase-content" [formGroup]="$any(phase)">
              ...
              <div class="dependencies-section">
                <div class="section-header">
                  ...
                  <button mat-icon-button (click)="addDependency(i)" matTooltip="Add dependency">
...
export class PhaseManagementDialogComponent implements OnInit {
  ...
  public dialogRef = inject(MatDialogRef<PhaseManagementDialogComponent>);
  public data = inject(MAT_DIALOG_DATA) as { projectId: string; phases: Phase[] };
  ...
  addExistingDependency(phaseGroup: FormGroup, dependency: { phaseId: string; type: string }) {
  ...
  getAvailablePhasesForDependency(currentPhaseIndex: number): { id: string; name: string }[] {
    // Can only depend on phases that come before this one
    return this.phasesArray.controls.slice(0, currentPhaseIndex).map((phase, index) => ({
  ...
  resetToDefaults() {
    if (
      this.phasesArray.length > 0 &&
      !confirm('This will replace all current phases with the default phases. Continue?')
    ) {
      return;
    }
    ...
    DEFAULT_PHASES.forEach((template, index) => {
      ...
      if (template.defaultDependencies) {
        template.defaultDependencies.forEach((dep: { phaseId: string; type: string }) => {
          const depIndex = DEFAULT_PHASES.findIndex(
            (p) => p.name.toLowerCase().replace(/\s+/g, '-') === dep.phaseId,
          );
