...
@Component({
    ...
    <div class="staff-list-container">
      ...
      <div class="filters">
        <mat-form-field appearance="outline">
          ...
          <input matInput [formControl]="searchControl" placeholder="Search by name, email, or ID">
      ...
      @if (loading()) {
        ...
        <div class="table-container">
          <table mat-table [dataSource]="filteredStaff()" matSort>
            ...
            <ng-container matColumnDef="availability">
              ...
              <td mat-cell *matCellDef="let staff">
                ...
                <div class="task-count">
                  {{ staff.availability.currentTaskCount }}/{{ staff.availability.maxConcurrentTasks }} tasks
                </div>
              </td>
            </ng-container>
              ...
              <td mat-cell *matCellDef="let staff">
                <div class="activity-info">
                  ...
                  @if (staff.activity.lastActive) {
                    <div class="last-active">Last active: {{ formatLastActive(staff.activity.lastActive) }}</div>
                  }
                </div>
              </td>
            </ng-container>
  ...
  styles: [`
    ...
    .group-admin { background: #e3f2fd; color: #1976d2; }
    .group-projectmanager { background: #f3e5f5; color: #7b1fa2; }
    .group-technician { background: #e8f5e9; color: #388e3c; }
    .group-supplier { background: #fff3e0; color: #f57c00; }
    .group-client { background: #fce4ec; color: #c2185b; }
  `]
})
export class StaffListComponent implements OnInit {
  ...
  displayedColumns = ['name', 'email', 'phone', 'group', 'availability', 'activity', 'status', 'actions'];
  ...
  private setupFilterSubscriptions() {
    ...
    this.groupControl.valueChanges.subscribe(() => this.applyFilters());
    this.statusControl.valueChanges.subscribe(() => this.applyFilters());
    this.activeControl.valueChanges.subscribe(() => this.applyFilters());
  }
  ...
  private applyFilters() {
    ...
    this.loading.set(true);
    this.staffService.getStaff(filter).subscribe({
      next: (staff) => {
        this.staff.set(staff);
        this.loading.set(false);
      },
  ...
  formatLastActive(timestamp: any): string {
