...
export class QualityService {
  ...
  private qualityCollection = collection(this.firestore, 'quality-metrics') as CollectionReference<QualityMetrics>;
  private checkpointsCollection = collection(this.firestore, 'quality-checkpoints') as CollectionReference<QualityCheckpoint>;
  ...
  getQualitySummary(projectId: string, startDate: Date, endDate: Date): Observable<QualitySummary> {
    ...
    return from(getDocs(q)).pipe(
      map(snapshot => {
        ...
        const avgSatisfaction = metrics.reduce((sum, m) => sum + (m.customerSatisfaction.satisfactionScore || 0), 0) / metrics.length;
        const avgSLA = metrics.reduce((sum, m) => sum + m.slaCompliance.overallScore, 0) / metrics.length;

        // Calculate trends
        const qualityScores = metrics.map(m => m.kpis.firstTimeRightRate || 0);
        const defectRates = metrics.map(m => m.defects.defectRate || 0);
        const satisfactionScores = metrics.map(m => m.customerSatisfaction.satisfactionScore || 0);
        ...
        metrics.forEach(m => {
          ...
          if (m.issues.critical > 0) allIssues.set('Critical Issues', (allIssues.get('Critical Issues') || 0) + m.issues.critical);
          if (m.issues.major > 0) allIssues.set('Major Issues', (allIssues.get('Major Issues') || 0) + m.issues.major);
          if (m.defects.verified > 0) allIssues.set('Verified Defects', (allIssues.get('Verified Defects') || 0) + m.defects.verified);
        });
        ...
        const topIssues = Array.from(allIssues.entries())
          ...
          .map(([description, frequency]) => ({
            description,
            frequency,
            impact: frequency > 10 ? 'high' : frequency > 5 ? 'medium' : 'low' as 'high' | 'medium' | 'low',
  ...
  private generateRecommendations(metrics: QualityMetrics[]): string[] {
    ...
    // Calculate averages
    const avgDefectRate = metrics.reduce((sum, m) => sum + (m.defects.defectRate || 0), 0) / metrics.length;
    const avgReworkRate = metrics.reduce((sum, m) => sum + m.rework.items, 0) / metrics.reduce((sum, m) => sum + m.inspections.completed, 0);
    const avgCompliance = metrics.reduce((sum, m) => sum + m.slaCompliance.overallScore, 0) / metrics.length;
    
    // Generate recommendations based on metrics
    if (avgDefectRate > 5) {
      recommendations.push('High defect rate detected. Consider additional training for installation teams.');
    }
    
    if (avgReworkRate > 0.1) {
      recommendations.push('Significant rework required. Review installation procedures and quality checkpoints.');
    }
    
    if (avgCompliance < 90) {
      recommendations.push('SLA compliance below target. Review response and resolution processes.');
    }
    
    // Check for safety issues
    const safetyIncidents = metrics.reduce((sum, m) => sum + (m.safetyQuality?.incidentsReported || 0), 0);
    if (safetyIncidents > 0) {
      recommendations.push('Safety incidents reported. Conduct safety review and additional toolbox talks.');
    }
