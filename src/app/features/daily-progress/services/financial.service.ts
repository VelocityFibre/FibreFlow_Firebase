...
export class FinancialService {
  ...
  private financialsCollection = collection(
    this.firestore,
    'project-financials',
  ) as CollectionReference<ProjectFinancials>;
    ...
    return from(getDocs(q)).pipe(
      map((snapshot) => {
        ...
        const totalCosts = financials.reduce(
          (sum, f) =>
            sum +
            (f.laborCost.total + f.materialCost.total + f.equipmentCost.total + f.otherCosts.total),
          0,
        );
