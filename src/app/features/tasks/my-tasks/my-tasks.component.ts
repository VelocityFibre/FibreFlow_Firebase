      ...
      <mat-tab-group class="task-tabs">
        ...
        <mat-tab>
          <ng-template mat-tab-label>
            <span [matBadge]="(activeTasks$ | async)?.length || 0" 
                  [matBadgeHidden]="(activeTasks$ | async)?.length === 0"
                  matBadgeColor="primary"
                  matBadgeSize="small">Active Tasks</span>
          </ng-template>
        ...
        <mat-tab>
          <ng-template mat-tab-label>
            <span [matBadge]="(completedTasks$ | async)?.length || 0" 
                  [matBadgeHidden]="(completedTasks$ | async)?.length === 0"
                  matBadgeColor="accent"
                  matBadgeSize="small">Completed</span>
          </ng-template>
          ...
          <div class="tab-content">
            <div class="task-list" *ngIf="completedTasks$ | async as tasks">
              ...
              <div *ngFor="let task of tasks" class="task-card completed" (click)="viewTask(task)">
                <div class="task-header">
                  <div class="task-info">
                    <h3>{{ task.name }}</h3>
                    <p class="task-project">{{ task.projectCode }} - {{ task.projectName }}</p>
...
export class MyTasksComponent implements OnInit {
  ...
  loadTasks() {
    ...
    this.tasks$ = combineLatest([
      ...
      map(([tasksObs, status, priority, project, assignee, search]) => tasksObs),
