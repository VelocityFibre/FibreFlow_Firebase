...
export class TaskAnalyticsService {
  ...
  generateAnalytics(tasks: Task[]): TaskAnalytics {
    ...
    const averageProgress = total > 0 ? Math.round(tasks.reduce((sum, t) => sum + (t.completionPercentage || 0), 0) / total) : 0;
    ...
    Object.keys(byProject).forEach(key => {
      ...
      project.completionRate = project.count > 0 ? Math.round((project.completed / project.count) * 100) : 0;
    });
    ...
    Object.keys(byAssignee).forEach(key => {
      ...
      assignee.completionRate = assignee.count > 0 ? Math.round((assignee.completed / assignee.count) * 100) : 0;
      assignee.averageProgress = assignee.count > 0 ? Math.round(assignee.totalProgress / assignee.count) : 0;
  ...
  private generateDailyTrends(tasks: Task[]): Array<{date: string, completed: number, created: number}> {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const completed = tasks.filter(t => {
        ...
        const completedDate = t.completedDate.toDate ? t.completedDate.toDate() : t.completedDate as any as Date;
        return completedDate.toISOString().split('T')[0] === dateStr;
      }).length;
      
      const created = tasks.filter(t => {
        if (!t.createdAt) return false;
        const createdDate = t.createdAt.toDate ? t.createdAt.toDate() : t.createdAt as any as Date;
  ...
  private generateWeeklyTrends(tasks: Task[]): Array<{week: string, progress: number}> {
    ...
    for (let i = 3; i >= 0; i--) {
      ...
      const progress = weekTasks.length > 0 ? 
        Math.round(weekTasks.reduce((sum, t) => sum + (t.completionPercentage || 0), 0) / weekTasks.length) : 0;
  ...
  private getFieldValue(task: Task, field: string): any {
    switch (field) {
      ...
      case 'month':
        if (task.createdAt) {
          const date = task.createdAt.toDate ? task.createdAt.toDate() : task.createdAt as any as Date;
