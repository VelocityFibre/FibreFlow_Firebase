...
@Component({
  ...
  template: `
    <div class="photo-capture-container">
      <h3>
        {{
          capturedPhotos().length > 0 ? 'Continue Capturing Photos' : 'Capture Photos (Optional)'
        }}
      </h3>
      ...
      @if (capturedPhotos().length > 0) {
        <div class="photo-status-summary">
          ...
          <span>{{ getUniquePhotoTypes() }} of {{ photoTypes.length }} photo types captured</span>
          @if (getUniquePhotoTypes() === photoTypes.length) {
            <mat-chip highlighted color="primary">Complete</mat-chip>
          } @else {
            <mat-chip>{{ photoTypes.length - getUniquePhotoTypes() }} types remaining</mat-chip>
          }
        </div>
      }
      ...
      <div class="photo-types">
        @for (config of photoTypes; track config.type) {
          ...
          >
            <mat-icon
              [matBadge]="getPhotoCount(config.type)"
              [matBadgeColor]="hasPhoto(config.type) ? 'accent' : 'warn'"
              matBadgeSize="small"
            >
              {{ config.icon }}
            </mat-icon>
            <span>{{ config.label }}</span>
            @if (hasPhoto(config.type)) {
        ...
        <mat-card class="preview-card">
          ...
          @if (compressionInfo()) {
            <div class="compression-info">
              <mat-chip-set>
                <mat-chip> Original: {{ formatSize(compressionInfo()!.originalSize) }} </mat-chip>
                <mat-chip [highlighted]="true">
                  Compressed: {{ formatSize(compressionInfo()!.compressedSize) }}
                </mat-chip>
                <mat-chip> Saved: {{ formatSavings() }}% </mat-chip>
              </mat-chip-set>
            </div>
          }
        </mat-card>
...
export class OfflinePhotoCaptureComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    // Always update captured photos when existing photos change
    this.capturedPhotos.set(this.existingPhotos);
  }

  ngOnChanges() {
    // Update captured photos if existingPhotos input changes after initialization
  ...
  getUniquePhotoTypes(): number {
    const uniqueTypes = new Set(this.capturedPhotos().map((p) => p.type));
