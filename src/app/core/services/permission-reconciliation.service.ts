...
export class PermissionReconciliationService {
  ...
  private async performReconciliation(
    ...
    try {
      ...
      for (const permission of unlinkedPermissions) {
        ...
        } else if (matches.length === 1 && matches[0].confidence < config.autoLinkConfidenceThreshold) {
  ...
  private async findPotentialMatches(
    ...
    for (const assignment of assignments) {
      ...
      // GPS proximity match
      if (permission.gpsLatitude && permission.gpsLongitude && 
      ...
      if (permission.fieldAgentPolePermission && assignment.oneMapData?.fieldAgent) {
        ...
        if (similarity >= config.agentNameSimilarityThreshold) {
          ...
          candidate.potentialConflicts.push(`Agent mismatch: ${permission.fieldAgentPolePermission} vs ${assignment.oneMapData.fieldAgent}`);
        }
      }
  ...
  private async createLink(
    ...
    const assignmentSnapshot = await getDocs(query(collection(this.firestore, 'planned-poles'), where('__name__', '==', match.assignmentId)));
   ...
   */
  private async createConflict(permission: OneMapRecord, matches: MatchCandidate[]): Promise<string> {
    const conflictType = matches.length > 1 ? ConflictType.MULTIPLE_MATCHES : ConflictType.AGENT_MISMATCH;
    ...
    const conflictDoc: Omit<PermissionConflict, 'id'> = {
      ...
      details: {
        agentNameMismatch: matches.some(m => m.potentialConflicts.some(c => c.includes('Agent mismatch'))),
        locationMismatch: matches.some(m => m.potentialConflicts.some(c => c.includes('GPS distance'))),
        ...
        suggestedAction: matches.length > 1 ? ConflictAction.REVIEW_MANUALLY : ConflictAction.AUTO_LINK,
  ...
  private async calculateStatusSummary(): Promise<LinkingStatusSummary> {
    ...
    const duplicateCount = links.filter(l => l.linkingStatus === LinkingStatus.DUPLICATE_POLE).length;
  ...
  private hasSignificantConflicts(conflicts: ConflictDetails): boolean {
    return conflicts.agentNameMismatch || 
