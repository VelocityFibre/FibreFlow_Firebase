...
class ProductionSyncService {
  ...
  async processRecord(stagingDoc) {
    try {
      ...
      // Track records without poles
      if (!stagingData.poleNumber) {
        if (!this.stats.noPoleRecords) this.stats.noPoleRecords = 0;
        this.stats.noPoleRecords++;
      }
  ...
  mapToPlannedPoles(data) {
    // Handle records without pole numbers - they still need to be tracked
    const poleNumber = data.poleNumber || 'PENDING_ASSIGNMENT';
    ...
    const mapped = {
      ...
      // Flag for field teams if no pole assigned
      requiresPoleAssignment: !data.poleNumber,
      assignmentPriority: !data.poleNumber && data.status === 'Pole Permission: Approved' ? 'HIGH' : 'NORMAL',
  ...
  mapToPoleTrackers(data) {
    // Pole trackers MUST have pole numbers - these are installed poles
    // Records without poles should go to planned-poles instead
    if (!data.poleNumber) {
      return null;
    }
  ...
  getProjectId(data) {
    // Map pole prefix to actual project IDs from live database
    const prefix = data.poleNumber?.split('.')[0] || 'LAW'; // Default to LAW if no pole
    
    // Project IDs from live FibreFlow database
    const projectMap = {
      'LAW': 'Law-001',  // Lawley project
      'MO': 'MO-001'     // Mohadin project
    };
    
    return projectMap[prefix] || 'Law-001'; // Default to Lawley
  }
...
Records without poles: ${this.stats.noPoleRecords || 0} (marked as PENDING_ASSIGNMENT)
