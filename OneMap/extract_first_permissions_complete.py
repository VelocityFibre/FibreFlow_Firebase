...
Uses the original CSV with all fields including contact numbers
"""
...
def extract_first_permissions():
    ...
    # Read all pole permission data from original CSV
    pole_data = defaultdict(list)
    
    # Read from the original data with all fields
        ...
        for row in reader:
            ...
            if 'Pole Permission' in row.get('Flow Name Groups', ''):
                ...
                if pole_num:
                    ...
                    date = row.get('Last Modified Pole Permissions Date', '') or row.get('Survey Date', '') or row.get('Date', '')
                    ...
                    pole_data[pole_num].append({
                        ...
                        'contact': row.get('Contact Number (e.g.0123456789)', ''),
                        'contact_name': row.get('Contact Person: Name', '') + ' ' + row.get('Contact Person: Surname', ''),
                        'stand_number': row.get('Stand Number', '')
                    })
    ...
    for pole_num, permissions in pole_data.items():
        ...
        valid_perms = [p for p in permissions if p['date']]
        
        if valid_perms:
            # Sort by date and take the first one
            sorted_perms = sorted(valid_perms, key=lambda x: x['date'])
            first_permissions[pole_num] = sorted_perms[0]
    
    # Save the first permissions only
    output_file = f'reports/{datetime.now().strftime("%Y-%m-%d")}_first_pole_permissions_complete.csv'
    ...
    summary_file = f'reports/{datetime.now().strftime("%Y-%m-%d")}_pole_permission_summary_complete.txt'
    ...
    with open(summary_file, 'w') as f:
        ...
        f.write(f"\n\nAgent Data Quality:\n")
        f.write(f"  Permissions with agent name: {agents_with_data} ({agents_with_data/len(first_permissions)*100:.1f}%)\n")
        f.write(f"  Missing agent name: {len(first_permissions) - agents_with_data} ({(len(first_permissions) - agents_with_data)/len(first_permissions)*100:.1f}%)\n")
        ...
        f.write(f"  Missing contact number: {missing_contact} ({missing_contact/len(first_permissions)*100:.1f}%)\n")
        f.write(f"  Missing GPS coordinates: {missing_gps} ({missing_gps/len(first_permissions)*100:.1f}%)\n")
    ...
    print(f"Poles with agent names: {agents_with_data}")
    print(f"Poles missing agent names: {len(first_permissions) - agents_with_data}")
    ...
    if duplicates_count > 0:
        print(f"\nDuplicate Analysis:")
        print(f"  Poles with payment conflicts: {duplicates_count}")
        print(f"  Poles with clean single claim: {len(first_permissions) - duplicates_count}")
        print(f"  Conflict rate: {duplicates_count/len(first_permissions)*100:.1f}%")
