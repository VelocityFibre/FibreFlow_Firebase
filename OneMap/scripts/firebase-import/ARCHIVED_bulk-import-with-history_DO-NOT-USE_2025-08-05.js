...
/**
 * Enhanced OneMap CSV Import with Status History Tracking
 * Preserves complete status change history for audit trails
 */
...
const KEY_FIELDS = [
  ...
  'Field Agent Name (Home Sign Ups)' // Added for photo tracking
];

// Import enhanced photo tracking functions
// TODO: Photo tracking module not yet implemented
// const { 
//   trackPhotoDetailsPerPole, 
//   logDetailedPhotoReport, 
//   displayPhotoResults 
// } = require('./enhanced-photo-tracking');

async function parseCSV(filePath) {
  return new Promise((resolve, reject) => {
    const records = [];
    const allValues = []; // Store all values for photo tracking
    ...
    rl.on('line', (line) => {
      ...
      if (record['Property ID']) {
        records.push(record);
        allValues.push(values); // Store for photo tracking
      }
    });
...
async function importCSVWithHistory(csvFileName) {
  try {
    console.log('🚀 Enhanced import with status history tracking...\n');
    ...
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      for (const record of chunk) {
        ...
        const statusEntry = {
          ...
          timestamp: importDate  // Use ISO string instead of serverTimestamp in array
        };
    ...
    // === ENHANCED PHOTO TRACKING (Updated 2025-07-31) ===
    // TODO: Photo tracking not yet implemented
    // const photoData = trackPhotoDetailsPerPole(records, csvFileName, allHeaders, allValues);
    // const reportFiles = logDetailedPhotoReport(photoData, csvFileName);
    ...
    // Display enhanced photo tracking results
    // TODO: Photo tracking not yet implemented
    // displayPhotoResults(photoData);
    // console.log(`\n   📄 Reports generated:`);
    // console.log(`      - Summary: ${path.basename(reportFiles.summaryLog)}`);
    // console.log(`      - Details: ${path.basename(reportFiles.detailReport)}`);
    // if (reportFiles.criticalReport) {
    //   console.log(`      - ⚠️  Critical: ${path.basename(reportFiles.criticalReport)}`);
    // }
...
if (!csvFile) {
  console.log('Usage: node bulk-import-with-history.js <csv-filename>');
  console.log('Example: node bulk-import-with-history.js "Lawley May Week 4 26052025.csv"');
  process.exit(1);
}

importCSVWithHistory(csvFile);